...
class MDQN(VanillaDQN):
    '''
    Implementation of MunchausenDQN with target network and replay buffer
    '''
    def __init__(self, cfg):
        ...
        self.alpha = cfg['agent']['log_r_scaling']
        self.clipping = cfg['agent']['log_r_clipping']
    ...
    def compute_q_target(self, batch):
        ...
        with torch.no_grad():
            ...
            tau_log_pi_cur_s = self.stable_scaled_log_softmax(q_cur)
            ...
            act_idx= batch.action.long().unsqueeze(1)
            tau_log_pi_a = tau_log_pi_cur_s.gather(1, act_idx).squeeze() #(bs,)
            tau_log_pi_a = tau_log_pi_a.clamp(self.clipping, 1)

            munchausen_term = self.alpha * tau_log_pi_a
            m_target= (batch.reward + munchausen_term + self.discount * expected_q_plus_entropy *  batch.mask)
