...
class VanillaDQN(BaseAgent):
  ...
  def run_steps(self, render=False):
    # Run for multiple episodes and test according to the step count
    self.step_count = 0
    self.episode_count = 0
    self.result = {'Train': [], 'Test': []}
    self.episode_return_list = {'Train': [], 'Test': []}
    # test before starting the training, as the evaluation result at step 0
    eval_average_episode_reward = self.run_test_episodes(render)
    self.logger.add_scalar(f'reward/eval_episodes_mean', eval_average_episode_reward, self.step_count)
    ...
    while self.step_count < self.train_steps:
      self.reset_game('Train')
      self.run_episode('Train', render)

  def run_episode(self, mode, render):
    while not self.done[mode]:
      self.action[mode] = self.get_action(self.state[mode], mode)
      if render:
      ...
      # suspend training, restart the training after testing, during 'run_test_episodes', the net mode have restored to 'Train'
      if self.step_count % self.test_interval_steps== 0:
        eval_average_episode_reward = self.run_test_episodes(render)
        if self.show_tb:
          self.logger.add_scalar(f'reward/eval_episodes_mean', eval_average_episode_reward, self.step_count)
        rolling_tr_score = np.mean(self.result[mode][-10:])
        self.logger.info(f'<{self.config_idx}>, total_tr_episodes: {self.episode_count}, total_step: {self.step_count},'
                         f'eval_episodes_mean: {eval_average_episode_reward}, train_last_10_mean: {rolling_tr_score}')

    self.result['Train'].append(self.episode_return[mode])
    self.episode_count += 1

  def run_test_episodes(self, render=False):
    mode = 'Test'
    self.set_net_mode(mode)
    total_episodes_reward_list = []
    for i in range(self.num_eval_episodes):
      ...
      total_episodes_reward_list.append(self.episode_return[mode])
    test_average_episode_reward = np.mean(total_episodes_reward_list)
    # reset the mode to 'Train', for continuing the training
    self.set_net_mode('Train')
  ...
  def get_action(self, input_state, mode='Train'):
    '''
    ...
    state = to_tensor(input_state, device=self.device)
  ...
  def learn(self):
    ...
    if self.gradient_clip > 0:
      grad_norm = nn.utils.clip_grad_norm_(self.Q_net[self.update_Q_net_index].parameters(), self.gradient_clip)
      # todo: log the grad norm
      if self.step_count % self.test_interval_steps == 0:
        self.logger.add_scalar(f'grad_norm', grad_norm, self.step_count)
    self.optimizer[self.update_Q_net_index].step()
    if self.show_tb and self.step_count % self.test_interval_steps == 0:
